...
function giveDetailUrl() {
  var e,
    t = $.address.value();
  if (
    -1 != t.indexOf("/" + portfolioKeyword + "/") &&
    t.length > portfolioKeyword.length + 2
  ) {
    var a = t.length;
...
function setCurrentMenuItem() {
  ...
  $("#header nav ul a").removeClass("waiting"),
    $("#header nav ul a[href$=" + e + "]")
      .parent()
      .addClass("current-menu-item")
      .siblings()
      .removeClass("current-menu-item"),
    $("select.mobile-nav option").attr("selected", ""),
    $("select.mobile-nav option[value$=" + e + "]").attr(
      "selected",
      "selected"
    );
}
...
function animateBars() {
  emptyBars(),
    $(".bar").each(function() {
      ...
      setTimeout(function() {
        e.find(".progress")
          .addClass("easing-long")
          .css("width", e.attr("data-percent") + "%");
      }, duration);
    });
}
...
$(function() {
  function e() {
    ...
    if (
      ...
      "" == (e = -1 != giveDetailUrl() ? portfolioKeyword : e))
    ) {
      var t = $("#header ul li")
        .first()
        .find("a")
    ...
    if (
      (Modernizr.csstransforms && Modernizr.csstransforms3d
        ? $("#" + e)
            .show()
            .removeClass("animated " + safeModPageOutAnimation)
            .addClass("animated " + safeModPageInAnimation)
        : ($("#" + e).fadeIn(), $(".page.active").hide()),
      $("#" + e).addClass("active"),
      setTimeout(function() {
        setCurrentMenuItem();
      }, 100),
      -1 != e.indexOf(portfolioKeyword) &&
        (setupPortfolio(),
        setTimeout(function() {
          setupPortfolio();
        }, 1e3)),
      $(".page.resume").hasClass("active") && (emptyBars(), animateBars()),
      $("body").scrollTop(0),
      $(".page.active .map iframe").length)
    ) {
  ...
  jQuery.browser.touch && $("html").addClass("touch"),
  ...
  var a =
    null != navigator.userAgent.match(/OS 5_[0-9_]+ like Mac OS X/i) ||
    null != navigator.userAgent.match(/OS 4_[0-9_]+ like Mac OS X/i) ||
    null != navigator.userAgent.match(/OS 3_[0-9_]+ like Mac OS X/i);
  (safeMod = "true" === $("html").attr("data-safeMod")),
    (mobileSafeMod =
      "true" === $("html").attr("data-mobileSafeMod") && jQuery.browser.mobile),
    (safeMod = safeMod || mobileSafeMod || a) &&
      ($("html")
    ...
    -1 == t || safeMod || $.address.path(portfolioKeyword),
    safeMod ||
      $("#pages").jmpress({
        stepSelector: ".page",
        fullscreen: !1,
  ...
  $("#header nav").prepend(
    '<select aria-label="navigation" class="mobile-nav"></select>'
  );
